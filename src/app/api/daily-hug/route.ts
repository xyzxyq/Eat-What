import { NextRequest, NextResponse } from 'next/server'
import { getSessionFromRequest } from '@/lib/auth'
import { prisma } from '@/lib/prisma'

// 获取今日日期字符串
function getTodayString(): string {
    const now = new Date()
    return now.toISOString().split('T')[0]
}

// 获取本月第一天
function getMonthStart(): string {
    const now = new Date()
    return `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-01`
}

// 计算连续打卡天数
async function calculateStreak(coupleSpaceId: string, type: string): Promise<number> {
    const allRecords = await prisma.dailyInteraction.findMany({
        where: { coupleSpaceId, type },
        select: { date: true },
        orderBy: { date: 'desc' },
        distinct: ['date']
    })

    if (allRecords.length === 0) return 0

    const dates = [...new Set(allRecords.map((r: { date: string }) => r.date))].sort().reverse()
    let streak = 0
    const today = getTodayString()

    for (let i = 0; i < dates.length; i++) {
        const expectedDate = new Date()
        expectedDate.setDate(expectedDate.getDate() - i)
        const expected = expectedDate.toISOString().split('T')[0]

        if (dates[i] === expected || (i === 0 && dates[0] === today)) {
            streak++
        } else if (i === 0 && dates[0] !== today) {
            // 今天还没打卡，从昨天开始算
            const yesterday = new Date()
            yesterday.setDate(yesterday.getDate() - 1)
            if (dates[0] === yesterday.toISOString().split('T')[0]) {
                streak++
            } else {
                break
            }
        } else {
            break
        }
    }

    return streak
}

// GET: 获取今日抱抱状态和统计
export async function GET(request: NextRequest) {
    try {
        const session = await getSessionFromRequest(request)
        if (!session) {
            return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        }

        const today = getTodayString()
        const monthStart = getMonthStart()

        // 获取今日所有抱抱记录
        const hugs = await prisma.dailyInteraction.findMany({
            where: {
                coupleSpaceId: session.coupleSpaceId,
                type: 'hug',
                date: today
            },
            include: {
                user: {
                    select: { id: true, nickname: true, displayName: true, avatarEmoji: true }
                }
            }
        })

        const currentUserHugged = hugs.some((h: { userId: string }) => h.userId === session.userId)
        const partnerHug = hugs.find((h: { userId: string }) => h.userId !== session.userId)
        const bothHugged = hugs.length >= 2

        // 获取统计数据
        const allHugRecords = await prisma.dailyInteraction.findMany({
            where: {
                coupleSpaceId: session.coupleSpaceId,
                type: 'hug'
            },
            select: { date: true },
            distinct: ['date'],
            orderBy: { date: 'desc' }
        })

        const thisMonthRecords = allHugRecords
            .filter((r: { date: string }) => r.date >= monthStart)
            .map((r: { date: string }) => r.date)

        const streak = await calculateStreak(session.coupleSpaceId, 'hug')

        return NextResponse.json({
            currentUserHugged,
            partnerHugged: !!partnerHug,
            partnerInfo: partnerHug?.user || null,
            bothHugged,
            stats: {
                streak,
                total: allHugRecords.length,
                thisMonth: thisMonthRecords
            }
        })

    } catch (error) {
        console.error('Get hug status error:', error)
        return NextResponse.json({ error: 'Internal error' }, { status: 500 })
    }
}

// POST: 记录抱抱
export async function POST(request: NextRequest) {
    try {
        const session = await getSessionFromRequest(request)
        if (!session) {
            return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
        }

        const today = getTodayString()

        // 检查是否已经抱过
        const existing = await prisma.dailyInteraction.findUnique({
            where: {
                type_date_userId: {
                    type: 'hug',
                    date: today,
                    userId: session.userId
                }
            }
        })

        if (existing) {
            return NextResponse.json({
                error: '今天已经抱过啦 🤗',
                alreadyHugged: true
            }, { status: 400 })
        }

        // 记录抱抱
        await prisma.dailyInteraction.create({
            data: {
                type: 'hug',
                date: today,
                userId: session.userId,
                coupleSpaceId: session.coupleSpaceId
            }
        })

        // 检查双方是否都抱了
        const allHugs = await prisma.dailyInteraction.findMany({
            where: {
                coupleSpaceId: session.coupleSpaceId,
                type: 'hug',
                date: today
            }
        })

        const bothHugged = allHugs.length >= 2

        // 获取更新后的统计
        const allHugRecords = await prisma.dailyInteraction.findMany({
            where: {
                coupleSpaceId: session.coupleSpaceId,
                type: 'hug'
            },
            select: { date: true },
            distinct: ['date']
        })

        const monthStart = getMonthStart()
        const thisMonthRecords = allHugRecords
            .filter((r: { date: string }) => r.date >= monthStart)
            .map((r: { date: string }) => r.date)

        const streak = await calculateStreak(session.coupleSpaceId, 'hug')

        return NextResponse.json({
            success: true,
            message: bothHugged ? '🤗 双方都抱抱啦！' : '🫂 已送出抱抱，等待 TA 回应~',
            bothHugged,
            stats: {
                streak,
                total: allHugRecords.length,
                thisMonth: thisMonthRecords
            }
        })

    } catch (error) {
        console.error('Hug error:', error)
        return NextResponse.json({ error: 'Internal error' }, { status: 500 })
    }
}
